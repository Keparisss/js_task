const data=[
  ...
  function firstNameSort(){
    data.sort(function(a,b){
      const el1=a.name.firstName;
      const el2=b.name.firstName;
      if (el1>el2) return 1;
      if (el1<el2) return -1;
      else return 0;
    })
  ...
  // сортировка фамилии по возрастанию
  function lastNameSort(){
    data.sort(function(a,b){
      const el1=a.name.lastName;
      const el2=b.name.lastName;
      if (el1>el2) return 1;
      if (el1<el2) return -1;
      else return 0;
    })
  ...
  // сортировка номера по возрастанию
  function phoneSort(){
    data.sort(function(a,b){
      const el1=a.phone;
      const el2=b.phone;
  ...
  // сортировка описания по возрастанию
  function aboutSort(){
    data.sort(function(a,b){
      const el1=a.about;
      const el2=b.about;
  ...
  // сортировка цвета глаз по возрастанию
  function eyeSort(){
    data.sort(function(a,b){
      const el1=a.eyeColor;
      const el2=b.eyeColor;
      if (el1>el2) return 1;
      if (el1<el2) return -1;
      else return 0;
    })
  ...
  // появление формы редактирования
  ...
  const eyeEdit=document.getElementById('eyeEdit');
  function setVisible(event){
    editForm.classList.add('visible');
    var myEl=event.target.closest('.string');
    //как получить элемент чтобы использовать id
    tapedString=data.find(el=> el.id===myEl.dataset.id);
    firstNameEdit.value=tapedString.name.firstName;
    lastNameEdit.value=tapedString.name.lastName;
    phoneEdit.value=tapedString.phone;
    aboutEdit.value=tapedString.about;
    eyeEdit.value=tapedString.eyeColor;
    // alert(myEl.dataset.id);
  ...
  // добавление слушателей для сортировки
  firstName.addEventListener("click",firstNameSort,false);
  lastName.addEventListener("click",lastNameSort,false);
  phone.addEventListener("click",phoneSort,false);
  about.addEventListener("click",aboutSort,false);
  eye.addEventListener("click",eyeSort,false);
...
function show(){
  data.forEach(element => {
    ...
    table.insertBefore(string,toplace);
    string.dataset.id=element.id;
    string.addEventListener('click',setVisible,false);
    ArrayForSort.push(string);

    // создание отдельного элемента
    const firstName=document.createElement('div');
    firstName.classList.add('item');
    string.appendChild(firstName);
    firstName.innerHTML= element.name.firstName;

    const lastName=document.createElement('div');
    lastName.classList.add('item');
    string.appendChild(lastName);
    lastName.innerHTML= element.name.lastName;

    const phone=document.createElement('div');
    phone.classList.add('item');
    string.appendChild(phone);
    phone.innerHTML= element.phone;

    const about=document.createElement('div');
    about.classList.add('item');
    string.appendChild(about);
    const str= element.about.slice(0,25);
    about.innerHTML= str + '...';

    const eyeColor=document.createElement('div');
    eyeColor.classList.add('item');
    string.appendChild(eyeColor);
    eyeColor.innerHTML= element.eyeColor;
  });
}
function deleteTableContent(){
  ArrayForSort.forEach(element=>{
    element.remove();
  })
}
